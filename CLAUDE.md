...
`emp-script-ai` 是 `com.gdxsoft.easyweb` 生态下的 Java 17 工具库，统一封装多家大模型服务商（OpenAI / Anthropic / Gemini / 通义千问 Qwen / 豆包 Doubao / Grok / 腾讯 Tencent / DeepSeek / OpenRouter）以及通用 OpenAI / Anthropic 兼容模式（自定义 URL）。通过 `RequestDataFactory` / `RequestAIFactory` 创建 provider-agnostic 请求体；通过 `AiStreamOrPost` 提供 SSE 流式与非流式 POST 两种调用方式。`switchproxy/` 子模块提供独立运行的 LLM 代理转发/路由服务（含 CLI）。`app/chatroom/` 是基于 WebSocket 的聊天室子应用。
...
- `pom.xml` — 默认构建，对应 `javax.servlet`（兼容传统 Servlet 容器）。
- `pomjdk17.xml` — Jakarta（`jakarta.servlet`，JDK 17+ / Tomcat 10+）构建。
...
├── switchproxy/           # 独立运行的 LLM 代理/路由服务（含 SwitchServer/SwitchCli）
│   ├── SwitchServer / SwitchCli / SwitchConfig   # 服务端 + CLI 启动入口
│   ├── ProfileConfig / RouteConfig / AccessKeyConfig  # 多 profile / 路由 / 访问密钥
│   ├── converter/ handler/ entry/ logger/        # 协议转换、请求处理、入口、日志
│   └── IpAccessController                        # IP 访问控制
└── app/
    └── chatroom/          # 基于 WebSocket 的聊天室子应用（ServerSdk/ClientSdk/HandleChatImpl 等）
```
...
| `SwitchServer` / `SwitchCli` | `switchproxy` 子模块独立代理服务的启动入口 |
...
- Java 17（`temurin`），UTF-8。日志走 SLF4J（实现使用 `logback-classic`，已替换早期 `slf4j-jdk14`），生产环境务必脱敏 API Key。
...
- `.qwen/skills/` — 自动提取的 agent skill，遇到对应场景应优先查阅：
  ...
  - `auto-skill-dual-servlet-profiles/` — javax/jakarta 双 servlet profile 配置
  - `auto-skill-switchproxy-module/` — `switchproxy` 代理模块的配置与扩展
...
- `SWITCH_DEV_GUIDE.md` — `switchproxy` 代理模块开发指南