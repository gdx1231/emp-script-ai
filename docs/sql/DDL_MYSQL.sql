-- =====================================================================
-- 视频创作工作流 DDL — MySQL (InnoDB, utf8mb4)
-- 全部 11 张表, 完整建库模版
-- =====================================================================

CREATE TABLE AI_WF_DEF (
    WFD_ID      BIGINT AUTO_INCREMENT PRIMARY KEY,             -- 主键
    WFD_NAME    VARCHAR(100) NOT NULL,                         -- 工作流名称
    WFD_DESC    VARCHAR(500),                                  -- 描述
    WFD_VERSION VARCHAR(20),                                   -- 版本
    WFD_JSON    LONGTEXT,                                      -- 完整 workflow.json
    WFD_MD5     VARCHAR(32),                                   -- JSON MD5(去重)
    WFD_STATUS  VARCHAR(20) DEFAULT 'USED',                    -- USED/DISABLED
    WFD_CDATE   DATETIME DEFAULT CURRENT_TIMESTAMP,            -- 创建时间
    WFD_MDATE   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- 修改时间
    INDEX idx_wfd_name (WFD_NAME, WFD_STATUS)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_WF_INSTANCE (
    WFI_ID          BIGINT AUTO_INCREMENT PRIMARY KEY,        -- 主键
    WFD_ID          BIGINT NOT NULL,                           -- FK→AI_WF_DEF
    WFI_UID         CHAR(36) NOT NULL,                        -- UUID
    ...
    WFI_INPUT       LONGTEXT,                                  -- 用户输入(故事文本)
    WFI_STORYBOARD  LONGTEXT,                                  -- 分镜JSON(Phase1输出)
    WFI_RESULT      LONGTEXT,                                  -- 最终结果JSON
    WFI_FINAL_URL   VARCHAR(500),                              -- 最终视频URL
    WFI_ERROR       LONGTEXT,                                  -- 失败原因
    ...
    USR_ID          INT,                                       -- 用户
    SUP_ID          INT,                                       -- 供应商
    WFI_CDATE       DATETIME DEFAULT CURRENT_TIMESTAMP,        -- 创建时间
    WFI_MDATE       DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- 修改时间
    WFI_SDATE       DATETIME,                                  -- 开始执行时间
    WFI_EDATE       DATETIME,                                  -- 完成时间
    WFI_ENGINE_ID   VARCHAR(100),                              -- 执行引擎标识(hostname+pid)
    INDEX idx_wfi_status (WFI_STATUS, WFI_PRIORITY, WFI_CDATE),
    INDEX idx_wfi_uid (WFI_UID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_WF_TASK (
    WFT_ID            BIGINT AUTO_INCREMENT PRIMARY KEY,       -- 主键
    ...
    WFT_INPUT         LONGTEXT,                                 -- 任务输入JSON
    WFT_OUTPUT        LONGTEXT,                                 -- 任务输出JSON
    ...
    WFT_ERROR         LONGTEXT,                                 -- 失败原因
    WFT_RETRY_COUNT   INT DEFAULT 0,                            -- 已重试次数
    WFT_CDATE         DATETIME DEFAULT CURRENT_TIMESTAMP,       -- 创建时间
    WFT_MDATE         DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- 修改时间
    WFT_SDATE         DATETIME,                                 -- 开始执行时间
    WFT_EDATE         DATETIME,                                 -- 完成时间
    INDEX idx_wft_instance (WFI_ID, WFT_TYPE, WFT_SEQ),
    INDEX idx_wft_status (WFT_STATUS),
    INDEX idx_wft_remote (WFT_REMOTE_ID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_CHAT (
    AI_ID        BIGINT AUTO_INCREMENT PRIMARY KEY,             -- 主键
    AI_UID       CHAR(36) NOT NULL,                             -- 请求编号
    AI_PROVIDER  VARCHAR(20) NOT NULL,                          -- AI供应商
    AI_MODEL     VARCHAR(50) NOT NULL,                          -- 模型
    AI_THINKING  TINYINT(1),                                    -- 思考模式
    AI_STREAM    TINYINT(1),                                    -- 流模式
    ...
    INDEX idx_ai_chat_ref (AI_REF, AI_REF_ID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_CHAT_MSG (
    AIM_ID               BIGINT AUTO_INCREMENT PRIMARY KEY,     -- 主键
    ...
    AIM_BY_USER          TINYINT(1) DEFAULT 0,                  -- 用户提问
    AIM_SKIP_APPEND      TINYINT(1),                             -- 下次回话不附加
    AIM_MSG              LONGTEXT NOT NULL,                      -- 消息内容
    ...
    AIM_TIME_END         DATETIME                                -- 结束时间
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_CHAT_EXP_ATTS (
    FILE_ID        BIGINT AUTO_INCREMENT PRIMARY KEY,            -- 主键
    ...
    FILE_UP_UA     LONGTEXT,                                     -- 上传UA
    FILE_UP_IP     VARCHAR(40),                                  -- 上传IP
    FILE_UP_JSP    LONGTEXT,                                     -- 上传JSP
    SUP_ID         INT,                                          -- 供应商
    ADM_ID         INT,                                          -- 管理员
    FILE_MDATE     DATETIME,                                     -- 修改时间
    FILE_CDATE     DATETIME DEFAULT CURRENT_TIMESTAMP            -- 创建时间
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE AI_CHAT_PARAMS (
    AIP_ID   BIGINT AUTO_INCREMENT PRIMARY KEY,                  -- 主键
    AI_ID    BIGINT NOT NULL,                                     -- FK→AI_CHAT
    AIM_ID   BIGINT NOT NULL,                                     -- FK→AI_CHAT_MSG
    AIP_NAME VARCHAR(100) NOT NULL,                              -- 参数名
    AIP_VAL  LONGTEXT,                                            -- 参数值
...
CREATE TABLE AI_CREATE_VIDEO_LIB (
    ...
    ACVL_UP_UA    LONGTEXT,                                       -- 上传UA
    ACVL_UP_IP    VARCHAR(40),                                    -- 上传IP
    ACVL_UP_JSP   LONGTEXT,                                       -- 上传JSP
    ACVL_CDATE    DATETIME,                                       -- 创建时间
    ACVL_MDATE    DATETIME,                                       -- 修改时间
...
CREATE TABLE AI_VOICE_CLONE (
    AVC_ID          BIGINT AUTO_INCREMENT NOT NULL,                  -- 主键
    AVC_VOICE_ID    VARCHAR(200) NULL,                               -- 音色 ID（API 返回）
    AVC_PROVIDER    VARCHAR(50) NULL,                                -- provider 类型
    AVC_TARGET_MODEL VARCHAR(100) NULL,                              -- 绑定的合成模型
    AVC_PREFIX      VARCHAR(100) NULL,                               -- 克隆时的前缀/名称
    AVC_AUDIO_URL   VARCHAR(500) NULL,                               -- 源音频 URL
    AVC_DESC        VARCHAR(500) NULL,                               -- 备注
    AVC_STATUS      VARCHAR(20) DEFAULT 'USED',                      -- USED/DELETED
    ADM_ID          INT NULL,                                        -- 管理员
    USR_ID          INT NULL,                                        -- 用户
    SUP_ID          INT NULL,                                        -- 供应商
    AVC_CDATE       DATETIME DEFAULT CURRENT_TIMESTAMP,              -- 创建时间
    AVC_MDATE       DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- 修改时间
    PRIMARY KEY (AVC_ID),
    INDEX IDX_AVC_VOICE_ID (AVC_VOICE_ID, AVC_STATUS),
    INDEX IDX_AVC_PROVIDER (AVC_PROVIDER, AVC_STATUS),
    INDEX IDX_AVC_TARGET_MODEL (AVC_TARGET_MODEL, AVC_STATUS)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
