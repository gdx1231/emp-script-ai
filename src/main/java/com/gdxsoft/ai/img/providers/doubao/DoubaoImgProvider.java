 ...
 * <p>
 * Doubao's API is OpenAI-compatible in response shape ({@code data[].url}) but
 ...
 * </ul>
 * <p>
 * Supported models: {@code doubao-seedream-5-0-260128},
 * {@code doubao-seedream-4.0-250828}, {@code doubao-seedream-3.0-t2i-250828}
 *
 * @since 1.2.0
 */
public class DoubaoImgProvider extends ImgProviderBase {
	...
	public ImgResponse generate(ImgRequest request) throws IOException, InterruptedException {
		if (apiKey == null || apiKey.isEmpty()) {
			throw new IllegalStateException("Doubao image generation requires an API key (Volcengine Ark API Key)");
		}
	...
	public String curl(ImgRequest request) {
		...
		sb.append("  -H 'Authorization: Bearer " + this.apiKey + "' \\\n");
	 ...
	 * <p>
	 * Follows the OpenAI-compatible shape but adds Doubao-specific fields. Visible
	 * for testing.
	 */
	public JSONObject buildRequestBody(ImgOptions opts) {
		...
		if (opts.getNegativePrompt() != null) {
			body.put("negative_prompt", opts.getNegativePrompt());
		}
		if (opts.getSeed() != null) {
			body.put("seed", opts.getSeed());
		}
		// Doubao-specific parameters
		body.put("watermark", watermark);
		body.put("stream", stream);

		// Sequential image generation (doubao seedream 5.0)
		String seqGen = opts.getSequentialImageGeneration();
		if (seqGen != null) {
			body.put("sequential_image_generation", seqGen);
			JSONObject seqOpts = opts.getSequentialImageGenerationOptions();
			if (seqOpts != null && !seqOpts.isEmpty()) {
				body.put("sequential_image_generation_options", seqOpts);
			}
		}

		// Output format (doubao seedream 5.0)
		if (opts.getOutputFormat() != null) {
			body.put("output_format", opts.getOutputFormat());
		}

		// Image-to-image: reference image(s)
		// Resolve effective URLs: prefer refImageUrls (plural), fall back to refImageUrl (singular).
		// Doubao's `image` field accepts a single URL string OR an array of URLs for multi-reference I2I
		// (supported by doubao-seedream-3.0 / 4.0 / 5.0). Single URL → string, multiple → array.
		java.util.List<String> refUrls = resolveRefImages(opts);
		if (refUrls != null && !refUrls.isEmpty()) {
			if (refUrls.size() == 1) {
				body.put("image", refUrls.get(0));
			} else {
				body.put("image", new org.json.JSONArray(refUrls));
			}
	 ...
	 * <p>
	 * Doubao's response is OpenAI-shaped:
	 * {@code {"created":..., "data":[{"url":"..."}]}}. We delegate to the OpenAI
	 * parser.
	 */
	...
	/**
	 * SSE streaming mode — reads events line by line without buffering the full
	 * response. Format: {@code data: {"created":..., "data":[...]}\n\n} Terminates
	 * with: {@code data: [DONE]}
	 */
	private ImgResponse generateStreaming(JSONObject body) throws IOException, InterruptedException {
		...
		HttpRequest.Builder builder = HttpRequest.newBuilder(URI.create(apiUrl))
				...
				.header("Accept", "text/event-stream").POST(HttpRequest.BodyPublishers.ofString(body.toString()));
