...
/**
 * AiStreamOrPost 负责管理 AI 聊天请求的初始化与处理，支持流式和非流式（POST）两种模式。
 * 它协调聊天管理器、AI 请求/响应处理以及输出机制之间的交互。
 * <p>
 * 主要职责包括：
 * <ul>
...
public class AiStreamOrPost {
    ...
    private boolean handleNoPromptsCase() {
        if (!chatManager.isNew()
            ...
            try {
                ...
                JSONObject rst = UJSon
                        .rstFalse("stepName=" + chatManager.getStepName() + ", " + getMessage("ACTION_EXECUTION_ERROR")
                                + err.getMessage());
                this.lastError = rst;
                chatManager.outEvent(rst.toString());
            }
    ...
    private String executeRequest(IRequestAI req, IRequestData reqData) throws Exception {
        ...
        if (chatManager.getStep().isStream()) {
            ...
            handleNonStreamResponse(req, fullText, writer);
        }
        return fullText;
    }
     ...
     */
    private void handleNonStreamResponse(IRequestAI req, String fullText, PrintWriter writer) {
        ...
        this.chatManager.outEvent(rst.toString());
    }
     ...
     * @param aimId AI消息记录ID
     */
    private void handleAiRequestException(IRequestAI req, Exception err, long aimId,
            String fullText) {
        if (aimId > 0) {
