...
public class TestEnq {
    public static JSONArray extractTravelDataToJson(String input) {
        ...
        // Regex patterns
        Pattern dayPattern = Pattern.compile("## 第(\\d+)天：.*? <cid>(\\d+)</cid>(?: <cid>(\\d+)</cid>)?");
        Pattern enjPattern = Pattern.compile("<enj>(.*?)</enj>", Pattern.DOTALL);
        Pattern idPattern = Pattern.compile("<id>(\\d+|未提供)</id>");

        for (int i = 0; i < days.length; i++) {
            String dayData = days[i].trim();

            JSONObject dayObject = new JSONObject();
            dayObject.put("ENQ_JNY_DAY", i + 1);

            // Extract cids
            Matcher dayMatcher = dayPattern.matcher(dayData);
            if (dayMatcher.find()) {
                dayObject.put("F_ADD", dayMatcher.group(2));
                dayObject.put("TO_ADD", dayMatcher.group(3) != null ? dayMatcher.group(3) : dayMatcher.group(2));
                dayObject.put("CITY_ID", dayMatcher.group(3) != null ? dayMatcher.group(3) : dayMatcher.group(2));
            }

            // Extract enj
            Matcher enjMatcher = enjPattern.matcher(dayData);
            if (enjMatcher.find()) {
                dayObject.put("ENQ_JNY_CONTENTN", enjMatcher.group(1).trim());
            }

            // Extract service connection IDs
            JSONArray serConns = new JSONArray();
            Matcher idMatcher = idPattern.matcher(dayData);
            int idIndex = 0;
    ...
    public static void main(String[] args) {
        ...
        JSONArray jsonResult = extractTravelDataToJson(input);
