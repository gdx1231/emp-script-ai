...
- `img/`、`video/`、`stt/` — 图像/视频/语音转文字子模块，各自有 `I*Provider`、`*Client`、`*ProviderFactory`、`*ProviderType` 和 `providers/` 实现（img: qwen/doubao/grok/openai/openaiCompat/stability；video: qwen/jimeng/kling/openaiCompat；stt: openai/openaiCompat/azure/google/qwen/local（qwen 走 DashScope 兼容模式 chat/completions + input_audio，模型 qwen3-asr-flash））。
